/**
@file
Arduino library for communicating with Modbus masters over RS232/485 (via RTU protocol).

@defgroup setup ModbusSlave Object Instantiation/Initialization
@defgroup buffer ModbusSlave Buffer Management
...
/*
  ...
  ModbusSlave.h - Arduino library for communicating with Modbus slaves
...
*/
#define __MODBUSSLAVE_DEBUG__ (0)
...
class ModbusSlave
{
  public:
    ...
    ModbusSlave(uint8_t, uint8_t, uint8_t);  // MJT
	
	void addBlock(reg_struct);
	// void addBlock(RegisterBlock.reg_struct); 
    void begin();
    void begin(uint16_t);
    void idle(void (*)());
	void listen();
    ...
    */
    ...
    /**
    ModbusSlave invalid response slave ID exception.
    ...
    */
    ...
    /**
    ModbusSlave invalid response function exception.
    ...
    */
    ...
    /**
    ModbusSlave response timed out exception.
    
    The entire response was not received within the timeout period, 
    ModbusSlave::ku8MBResponseTimeout. 
    ...
    */
    ...
    /**
    ModbusSlave invalid response CRC exception.
    ...
    */
	...
	void     flushReadBuffer();   // MJT, flushes Serial recieve buffer
	...
	reg_struct _list_blocks[10];
  private:
	// reg_struct _list_blocks[10];

	uint8_t _u8NumBlocks                                   = 0; // number of blocks passed to class 
    ...
    // static uint16_t ku16MBResponseTimeout;  //          = 2000; ///< Modbus timeout [milliseconds]
    
    // master function that conducts Modbus transactions
    // uint8_t ModbusSlaveTransaction(uint8_t u8MBFunction);
	
	void sendResponse(uint8_t, uint8_t);
	uint8_t checkRegisters();
	void setPinTx();
	void setPinRx();
