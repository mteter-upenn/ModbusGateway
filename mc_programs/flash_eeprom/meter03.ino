uint16_t meter3(uint16_t mtr_start){
  uint16_t blk_strt;
  uint8_t blk_num;
  uint16_t grp_strt;
  uint8_t grp_num;
  ...
  EEPROM.write(mtr_start + 6, highByte(grp_strt + 45));
  EEPROM.write(mtr_start + 7, lowByte(grp_strt + 45));
  ...
  EEPROM.write(blk_strt + 9, FloatConv2Int8(FloatConv::MOD1K));
