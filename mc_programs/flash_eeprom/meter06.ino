int meter6(uint16_t mtr_start){
  ...
  EEPROM.write(++grp_strt, int8_t(-10));  // 9
  ...
  EEPROM.write(++grp_strt, int8_t(-2));  // +24
  for (i = 18; i < 22; i++) { //
    EEPROM.write(++grp_strt, i);  // data type
  }
  EEPROM.write(++grp_strt, int8_t(-2));
  ...
  EEPROM.write(++grp_strt, int8_t(-4));  // +34
  ...
  EEPROM.write(++grp_strt, int8_t(FloatConv::UINT16));
  ...
  Serial.print(F(" to "));
  Serial.println(grp_strt, DEC);  // +78
