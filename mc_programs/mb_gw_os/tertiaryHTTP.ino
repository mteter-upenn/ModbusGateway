...
char * preprocPost(EthernetClient client, char * headHttp, uint16_t &postLen) {
  ...
  while (!foundPtr) {
    ...
    if (msgPtr != NULL) {
      ...
      msgPtr = headHttp + lenRead - 15;  // stick msgPtr at end

      for (msgPtr; msgPtr < headHttp + lenRead; msgPtr++) {
        for (i = 0; i < 16; i++) {
          if (*(msgPtr + i) == contLen[i]) {
            // continue to match headHttp and contLen
          }
          else if (*(msgPtr + i) == '\0') {  // will never be in position such that msgPtr + 0 == 0, therefore i must be at least 1
      ...
      if (charMatches) {
        // try to match beginning of headHttp with remainging chars of contLen
        for (i = charMatches; i < 16; i++) {
          if (contLen[i] == headHttp[i - charMatches]) {
            if (i == 15) {
              foundPtr = true;
              msgPtr = headHttp + 16 - charMatches;
            }
          }
          else {
            charMatches = 0;
...
void getPostSetupData(EthernetClient cl, char * headHttp) {
  ...
  startPostMsg = preprocPost(cl, headHttp, postLen);
